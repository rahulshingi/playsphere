...
# ============================================================
# Cricket — CricHeroes-style match flow (toss, XI, ball-by-ball)
# ============================================================
...
def _cricket_initial_state(overs_limit: int = 20) -> dict:
    return {
        "sport": "cricket",
        "match_state": "toss",            # toss -> playing_xi -> in_play -> innings_break -> in_play -> completed
        "overs_limit": overs_limit,
        "toss": None,
        "playing_xi": {"team_a": [], "team_b": []},
        "current_innings": 0,
        "innings": [],
        # Legacy summary fields (read by standings + UI cards)
...
def _overs_str(legal_balls: int) -> float:
    """Convert legal balls into decimal overs (e.g. 13 balls -> 2.1)."""
    return float(f"{legal_balls // 6}.{legal_balls % 6}")


def _which_team_key(score: dict, team_id: str) -> str:
    """Find whether a team is team_a or team_b by looking up the fixture-level mapping."""
    # batting team id is matched via the snapshot stored on innings
    if score.get("_team_a_id") == team_id:
        return "team_a"
    if score.get("_team_b_id") == team_id:
        return "team_b"
    return "team_a"


def _sync_summary_fields(score: dict, fixture_team_a_id: str, fixture_team_b_id: str):
    """Mirror per-innings totals into top-level team_a / team_b for standings + cards."""
...
async def cricket_setup(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    ...
    score = _cricket_initial_state(overs_limit)
...
async def cricket_playing_xi(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    """Body: {team_a: [{player_id, name, captain?, wk?}], team_b: [...] } -- variable size accepted."""
    fixture = await _get_fixture_or_404(fixture_id)
    score = fixture.get("score") or {}
    if not score or score.get("sport") != "cricket":
    ...
    # if both XIs have at least 2 players, allow user to proceed
...
async def cricket_start_innings(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    ...
    # Determine batting/bowling team for THIS innings
    a_id, b_id = fixture["team_a_id"], fixture["team_b_id"]
    if not score.get("innings"):
        # First innings — driven by toss
    ...
    # Target for chasing innings
    if score.get("innings"):
        inn["target"] = score["innings"][-1]["runs"] + 1

    # Resolve names from playing XI
    ...
    name_map = {p["player_id"]: p["name"] for p in xi_batting + xi_bowling}

    if striker_id not in name_map or non_striker_id not in name_map:
        raise HTTPException(400, "Striker/non-striker must be in batting XI")
    if bowler_id not in name_map:
        raise HTTPException(400, "Bowler must be in bowling XI")

    _add_batsman(inn, striker_id, name_map[striker_id])
    _add_batsman(inn, non_striker_id, name_map[non_striker_id])
    _add_bowler(inn, bowler_id, name_map[bowler_id])
...
# ---------- Cricket: record a ball ----------
@api.post("/fixtures/{fixture_id}/cricket/ball")
async def cricket_ball(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    """
    Body: {runs:int, extra: 'wd'|'nb'|'b'|'lb'|None, wicket: {type,'fielder_id'?,'new_batsman_id'?}|None, commentary:str?}
    runs = runs scored off bat (excluding the +1 wide/no-ball). For b/lb, runs are bye/leg-bye runs only.
    """
    fixture = await _get_fixture_or_404(fixture_id)
    score = fixture.get("score") or {}
    if score.get("sport") != "cricket":
    ...
    legal = True       # counts towards over count and ball faced
    bat_runs = 0       # runs added to striker
    team_runs = 0      # total runs added to innings score
    bowler_runs = 0    # runs against bowler (incl. wd/nb)
    ...
    if extra == "wd":
        ...
        team_runs = 1 + runs
        bowler_runs = 1 + runs
        bat_runs = runs   # runs off bat on no-ball are batsman's
        extras_inc["nb"] = 1
        if runs % 2 == 1:
            swap_strike = True
    elif extra == "b":
    ...
    # Apply to striker (only counts a ball faced for legal non-bye/leg-bye)
    ...
    if wicket and wicket.get("type") in DISMISSAL_TYPES:
        # On wides, batsman cannot be bowled/caught/lbw but can be runout/stumped
        wtype = wicket["type"]
        # Determine which batsman is out: default striker, runout may indicate non-striker via 'batsman_id'
        ...
        if out_player and not out_player["out"]:
            ...
            if wtype != "runout":
                bowler["wickets"] += 1
            # Clear the out batsman from the crease
            if inn["striker_id"] == out_pid:
                inn["striker_id"] = None
            elif inn["non_striker_id"] == out_pid:
                inn["non_striker_id"] = None
            score["match_state"] = "wicket"  # block further balls until /new-batter or innings ends
    ...
    # End-of-over: swap strike + bowler must change
    end_of_over = legal and inn["legal_balls"] % 6 == 0
    if end_of_over:
        # maiden detection: if no runs (bat or extras) credited to bowler in the over
        # Walk back through balls_log of this over
        over_balls = [b for b in inn["balls_log"] if b["over"] == over_num][-6:]
        bowler_runs_over = sum(b["team_runs"] if b["extra"] in ("wd", "nb") else b["runs"] for b in over_balls if b["bowler_id"] == bowler_id)
        if bowler_runs_over == 0:
            bowler["maidens"] += 1
        swap_strike = not swap_strike  # over-end flips strike
        # Force bowler change next
        inn["current_bowler_id"] = None

    # Strike swap (don't swap if striker just got out — new batter comes to striker by default)
...
# ---------- Cricket: new batsman after a wicket ----------
...
async def cricket_new_batsman(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    ...
    # Slot into whichever end is vacant (default striker)
...
# ---------- Cricket: new bowler at end of over ----------
...
# ---------- Cricket: end match (declares winner) ----------
...
async def cricket_end_match(fixture_id: str, body: dict, _: dict = Depends(require_admin)):
    ...
    if score.get("sport") != "cricket":
        raise HTTPException(400, "Not a cricket fixture")
    winner_id = body.get("winner_team_id")
    if winner_id not in (fixture.get("team_a_id"), fixture.get("team_b_id"), None, ""):
...
# ---------- Cricket: undo last ball ----------
@api.post("/fixtures/{fixture_id}/cricket/undo")
async def cricket_undo(fixture_id: str, _: dict = Depends(require_admin)):
    """Crude undo: removes the last ball log entry and rebuilds the innings from scratch.
    Acceptable since balls_log is the source of truth."""
    fixture = await _get_fixture_or_404(fixture_id)
    score = fixture.get("score") or {}
    if score.get("sport") != "cricket":
        raise HTTPException(400, "Not a cricket fixture")
    ...
    inn["balls_log"].pop()
    # Rebuild innings totals from balls_log
    inn["runs"] = 0
    inn["wickets"] = 0
    inn["legal_balls"] = 0
