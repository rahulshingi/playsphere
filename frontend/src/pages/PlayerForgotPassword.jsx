...
export default function PlayerForgotPassword() {
  ...
  const submit = async (e) => {
    ...
    setBusy(true);
    try {
      await api.post("/players/forgot-password", { email });
      setSent(true);
      toast.success("If the email matches, a reset link has been generated");
    } catch {
      ...
      <div className="max-w-md mx-auto px-6 pt-16 pb-24">
        ...
        {sent ? (
          <div className="mt-8 border border-white/10 rounded-sm bg-[#141414] p-5 text-sm text-neutral-300">
            ...
            <Link to="/players/login" className="text-[#84CC16] hover:underline text-xs mt-4 inline-block">← Back to sign in</Link>
          </div>
        ) : (
