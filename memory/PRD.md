...
- **Task 3 — Tabular vendor bookings**: `VendorDashboard.jsx` rewired to render the new `UnifiedBookingsTable.jsx` component in place of the ad-hoc inline table.
- **Task 4 — Unified bookings view**: merged `/vendor-bookings` (platform online) + `/vendor/private-bookings` (offline) into one sortable table with source chips (`PLATFORM` / `OFFLINE`), range filters (Today / This week / This month / All), status filters (Pending / Confirmed / Active / Completed / Expired / Cancelled), plus a header stat strip (total / active / done / no-show counts). Auto-expired rows land here via backend lazy sweep.
- **Task 5 — Show-up tracking + auto-expire**: 4 new endpoints — `POST /api/vendor-bookings/{id}/check-in` (vendor confirms customer arrived → `status=completed`), `POST /api/vendor-bookings/{id}/no-show` (early no-show mark), plus offline mirrors `POST /api/vendor/private-bookings/{id}/check-in` and `/no-show`. New `backend/routes/booking_lifecycle.py` runs lazy sweep on every list-bookings call: bookings whose `end_time + 4h grace` is in the past AND status ∈ (`pending`, `vendor_accepted`, `confirmed`, `active`) get flipped to `status=expired` with `no_show_at` stamped. Zero-cron design. UI: vendor gets green **Arrived** + red **No-show** buttons on every actionable row.
- **Model additions**: `Vendor.commission_percent`, `Vendor.commission_min_flat`. `VendorBooking.commission_min_flat`, `VendorBooking.checked_in_at`, `VendorBooking.checked_in_by`, `VendorBooking.no_show_at`, `VendorBooking.completed_at`. `PrivateBooking` gains same 4 show-up fields + `expired` status.
- **Tests**: `test_task44_bookings_analytics.py` (7/7 pass) — commission set/validate, analytics shape (day/week/month), check-in, no-show, completed-cannot-noshow, commission max-of-two rule. Aggregate 39 tests pass with 1 skipped, zero regressions.


## Implemented (Feb 27, 2026 — Iteration 43) 7-item UX batch fix
1. **Past slots hidden in booking calendar**: `/vendor-listings/{id}/availability` now marks slots that have already elapsed as `status="past"` for today's date — frontend renders them non-clickable. Future dates are unaffected. Fixes users being shown morning slots when booking in the afternoon.
2. **Sport-specific default event image**: new `SPORT_IMAGES` map + `sportImage(sport)` helper in `lib/sports.js` (cricket / badminton / tennis / snooker / basketball / pickleball / chess / hackathon etc.). `Events.jsx` card fallback + `EventDetail.jsx` hero banner + `Home.jsx` featured cards all now render the sport-themed Pexels photo instead of the generic football fallback.
3. **Start date + end date on event forms**: Admin.jsx (player-host + org-admin flows) + `admin/EventsTab.jsx` (platform admin) all now expose date inputs with `min={todayLocalISO()}` and end_date min bounded by start_date. Existing PATCH already accepts these fields; no backend change required.
4. **Cancel event (soft-cancel)**: `EventStatus` widened to include `"cancelled"`; new `POST /api/events/{id}/cancel` endpoint stores `cancellation_reason`, `cancelled_at`, `cancelled_by` and preserves fixtures/teams/photos for audit. `EventDetail.jsx` shows a red "Cancel event" button (creator/admin only, hidden once cancelled) and renders a cancelled banner with reason chip.
5. **Anonymous vendor calendar preview**: `VendorMarket.jsx` no longer redirects anonymous users to /login when they click a listing — they can browse the full booking modal + see the availability calendar. The Send Request button flips to a "Sign in to book" link for anonymous viewers, with role signup CTAs (HR / Organiser / Player) shown as a helper hint.
6. **Phone → Mobile number rename + sponsor mobile field**: `SignupCompany.jsx` + `SignupOrganiser.jsx` labels & test-ids updated to Mobile number (existing `contact_phone` payload preserved to avoid backend churn). `SignupSponsor.jsx` gains a Mobile number input; `SponsorSignupBody` accepts optional `mobile` which is stored on the User doc — rejects duplicate mobiles with 400. Users can log in with email or mobile (login-by-mobile was already shipped in iter40).
7. **Rotating homepage hero image**: `Home.jsx` picks the next sport from a 10-sport rotation (cricket → badminton → tennis → basketball → pickleball → football → tabletennis → snooker → volleyball → chess) on every fresh visit, persisted via `localStorage` counter with a `Date.now()` minute fallback for storage-blocked sessions.
- **Model**: `Event` gained `cancellation_reason`, `cancelled_at`, `cancelled_by`. `SponsorSignupBody` gained `mobile`.
- **Tests**: `tests/test_batch_feb2026_fixes.py` (9/9 pass) — past-slot filter (today & future), event cancel (mark, idempotent double-cancel, auth, 404), sponsor signup with mobile (accept, duplicate reject, optional). Zero regressions across `test_phase3_phase4` (14/14) + `test_phase5_sport_rules` (7/7) + `test_public_scorecard` (3/3) + `test_organiser_signup_otp` (8/8).


## Implemented (Feb 26, 2026 — Iteration 43) Dynamic Sports Config Phase 5 + Bracket View
- **Phase 5 — Sport-specific rules**: `LiveScorer.jsx` rewritten to be rule-aware — driven by new `SPORT_RULES` map + `computeMatchOutcome()` helper in `lib/sports.js`. Racket sports (badminton / TT / tennis / pickleball / squash / volleyball) render N set inputs sized to `best_of_sets` with per-set `points_to_win_set` + `deuce_win_by` + `hard_cap`. Snooker + pool render a single `frames_won` counter with `race_to_frames` displayed. Chess renders a `ChessResultEditor` with 3 radios (white / draw / black), auto-fills 1/½/0 game points, exposes result string in `score.result`. Every scorer shows rule chips ("Best of 3 · to 11 (win by 2)", "Race to 5 frames", "Time 10+5", "Tiebreak") + an outcome-hint banner ("Match won 2-1") + `Mark completed` button that auto-fills status + winner_id when the win condition is satisfied.
- **Backend**: `default_score()` extended to emit correct shapes per sport — pickleball [0,0,0], tabletennis [0,0,0,0,0], volleyball [0,0,0,0,0], snooker `{frames_won: 0}`, chess `{result: null}`. `_compute_totals()` in `routes/fixtures.py` also reads `frames_won` so snooker/pool winners auto-compute on `status=completed`.
- **Bracket View (Enhancement)**: new `/app/frontend/src/components/event/BracketView.jsx` — visual tournament bracket for knockout + double-elimination events. Single-elim: rounds-as-columns with score-per-side match cards, clickable to /live/{fixtureId}. Double-elim: three sections stacked — Winners Bracket (accent lime) + Losers Bracket (accent pink) + Grand Final (accent amber). Bracket tab TabsTrigger now shows for both `knockout` and `double_elimination`. Replaces the old lightweight `Bracket` component in `EventDetail.jsx`.
- **Bracket Share Card**: new `/app/frontend/src/lib/shareBracketImage.js` — canvas 2D pipeline producing 1080×1350 Instagram-story-ready PNG. Renders bracket columns for single-elim, WB+LB+GF stacked for double-elim, with team colour bars, winner highlights, and Kreeda Nation footer. `Share bracket` button on `BracketView` uses `navigator.canShare` on mobile (native sheet → WhatsApp/Instagram) and falls back to a browser PNG download on desktop.
- **Tests**: `tests/test_phase5_sport_rules.py` — 7/7 pass covering default_score for pickleball / tabletennis (best-of-5) / snooker (frames_won) / chess (result: null), snooker frames winner auto-fill, pickleball sets winner auto-fill, chess result persistence. Aggregate 55/55 green across Phase 3+4+5 + regression suites.
- **Frontend E2E (testing agent iteration_35)**: 11/11 flows PASS — pickleball scorer, snooker scorer, chess scorer, TT scorer, knockout bracket, double-elim bracket + share PNG, Swiss pair-next, FixtureMetaEditor persistence, TeamRolesEditor persistence, NewTeamForm regression, FixtureAwardsEditor regression. Zero P0/P1 bugs.


## Implemented (Feb 26, 2026 — Iteration 42) Dynamic Sports Config Phase 3 + Phase 4
- **Phase 3 — Team roles**: `Team` model extended with `vice_captain`, `vice_captain_player_id`, `coach_name`, `manager_name`, `jersey_color`. New endpoint `PATCH /api/events/{event_id}/teams/{team_id}` for organiser/HR/captain to set/clear these fields; setting `vice_captain_player_id` auto-resolves the linked player's display name and adds them to the roster if not already present. Legacy `PATCH /api/teams/{id}` (admin-only) unchanged.
- **Phase 3 — Match metadata**: `Fixture` model extended with `court_number`, `officials: List[{role, name}]`, `toss: {winner_team_id, decision, note}`. New endpoint `PATCH /api/fixtures/{fixture_id}/meta` for creator/admin — accepts partial payloads (venue, court_number, scheduled_at, officials, toss). Editable at any status (including completed) so organisers can retroactively correct match-day metadata. Rejects unknown toss winners (400) and bogus decisions (allow-list: bat/field/bowl/serve/receive/choose_side).
- **Phase 4 — Swiss fixtures**: `generate_swiss(team_ids, event_id, rounds)` produces concrete round-1 seeded pairings (top-half vs bottom-half) plus empty placeholder fixtures for later rounds (`SW-R{n}-M{n}` bracket positions). Round count defaults to `ceil(log2(N))`. New endpoint `POST /api/events/{event_id}/swiss/pair-next-round` reads current standings (2 pts win / 1 pt draw / 2 pts bye), ranks teams, and greedy-pairs adjacent ranks while avoiding rematches. Refuses to pair round N until round N-1 is completed.
- **Phase 4 — Double elimination**: `generate_double_elimination(team_ids, event_id)` builds a full bracket — Winners Bracket (WB-R{n}-M{n}), Losers Bracket (LB-R{n}-M{n}), and single Grand Final (GF-M1). Teams padded to next power-of-2; empty slots for LB/GF-later rounds are advanced via existing knockout propagation once a scorer completes a match.
- **`FixtureFormat` union widened** to accept `"swiss"` and `"double_elimination"`. Event creation dropdowns in `Admin.jsx`, `admin/EventsTab.jsx` gained the new options.
- **Frontend UI**:
  - `EventTeamsManager.NewTeamForm` now captures jersey colour + coach + manager at team-create time.
  - New `TeamRolesEditor` dialog (opened via pencil icon on `TeamCard`) — set VC (auto-adds to roster), coach, manager, jersey colour. TeamCard header renders VC/coach/manager chips + jersey-colour swatch.
  - New `/app/frontend/src/components/event/FixtureMetaEditor.jsx` — dialog for venue, court/table/lane, scheduled_at, officials (multi-row with role dropdown), toss winner + sport-aware decision list (cricket→bat/field, racket→serve/receive/choose_side). `FixtureMetaStrip` renders a compact chip line on every FixtureCard summarising the metadata.
  - "Pair next Swiss round" button appears on the fixtures tab for Swiss events, hitting the new pairing endpoint.
- **Tests**: `tests/test_phase3_phase4.py` (14/14 pass) — team meta CRUD, VC auto-resolve, VC clear, unknown VC rejection, fixture meta save + validation (bad toss winner 400, bad decision 400), Swiss round-1 seeding + placeholder round-N, Swiss pair-next requires completion, Swiss pair-next actually pairs, double-elim 4-team + 8-team bracket sizes. Zero regressions across `test_event_teams` / `test_public_scorecard` / `test_fixture_completion_iter34` / `test_cricket_scoring` / `test_share_and_sport_config_iter39` / `test_profile_stats_iter38` (51 pre-existing tests still green).
