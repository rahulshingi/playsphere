...
- ✅ Iter 52 (Jul 13, 2026): **Slot-conflict guard + guest browsing + Google clarification**. Audit of platform vs offline bookings confirmed the model is correct (`vendor_bookings` with `offline_source=true` for ref-link buyers, `private_bookings` for vendor walk-ins). Added server-side `_guard_slot_conflict()` — used by both `POST /vendor-bookings` and `POST /vendor/private-bookings`. Rejects (HTTP 409) any request that overlaps an existing platform booking, an offline walk-in, or an explicit vendor block. Availability endpoint (`GET /vendor-listings/{id}/availability`) already honours all three, so the UI slot picker greys out taken slots correctly. `VendorMarket` (/hire) now shows a **"Browsing as guest"** banner to unauthenticated visitors — filter, preview, and check availability without an account; only submit triggers login. `/login` Google button caption now clarifies "Signs you in as a Player. HR, Company, Organiser, Sponsor and Vendor accounts — please use the signup form."
