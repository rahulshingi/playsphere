...
- ✅ Iter 54 · Phase 1 (Jul 13, 2026): **Corporate Services (RFQ module) — foundations.** Removed the old `/services` public storefront and `/services/:id` detail (both routes now redirect to `/corporate-services`). New backend module `routes/corporate_services.py` with 12 admin CRUD endpoints across `service_categories`, `cs_services`, `cs_addons`, `cs_packages` collections (pricing intentionally excluded from Phase 1). New Platform Admin **"Corporate Services"** tab with 4 sub-tabs (Packages / Categories / Services / Add-ons) for full catalogue management including package composition (multi-select services + addons). New customer-facing `/corporate-services` page with role-gated catalogue view for HR/Organiser (other roles/anonymous see a soft-gate). Nav link "Corporate Services" injected into HR/Organiser "more" menu. Curl-verified end-to-end: create category → service → addon → package → hydrated public list.
- ✅ Iter 53 (Jul 13, 2026): Consolidated `slot_blocks` + `venue_blocks` collections.
- ✅ Iter 52 (Jul 13, 2026): Slot-conflict guard + guest browsing + Google clarification.
- ✅ Iter 51 (Jul 13, 2026): Google Sign-In (Emergent-managed OAuth).
- ✅ Iter 50 (Jul 13, 2026): Player universal profile — personal email signup + corporate email verification.
- ✅ Iter 49 (Jul 13, 2026): Commission invoices + overtime billing + booking reopen + memberships restructure.
