...
- ✅ Iter 51 (Jul 13, 2026): **Google Sign-In (Emergent-managed OAuth)**. Added `POST /api/auth/google/session` — receives Emergent session_id, exchanges with `auth.emergentagent.com`, then either signs in existing user (email match) or provisions a fresh `player` account (with matching `player_profiles` row). Issues our own JWT `access_token` cookie so the entire app keeps using the existing auth path. Frontend: white "Sign in with Google" button under existing form on `/login`, new `/auth/callback` page that reads `#session_id=…` from URL fragment, exchanges, refreshes AuthContext, and routes by role. Zero keys required — Emergent handles OAuth.
