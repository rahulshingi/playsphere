...
- ✅ Iter 53 (Jul 13, 2026): **Consolidated `slot_blocks` + `venue_blocks` collections**. Previously two collections referred to the same concept — vendor's Offline-mode UI wrote to `slot_blocks` while the Listings tab used `venue_blocks`, and the availability guard only checked `venue_blocks`. Both endpoints now read/write the unified `venue_blocks` collection, so a block created from either UI blocks both booking flows. One-time migration script `scripts/migrate_slot_blocks.py` merged 4 legacy rows and dropped the `slot_blocks` collection. Curl-verified: create via `/vendor/slot-blocks` → visible via `/vendor-listings/{lid}/blocks` → availability endpoint shows the slot as blocked.
