...
- ✅ Iter 55 · Phase 2 (Jul 13, 2026): **Corporate Services · HR customiser + RFQ submit + My Requests.** Backend `POST /rfqs`, `GET /rfqs/mine`, `GET /rfqs/{id}`, `POST /rfqs/{id}/cancel` — HR-role-gated with 403 for players/vendors. RFQ snapshots the package + selected services + addon quantities so later catalogue edits can't mutate a submitted request. `cs_status_history` audit trail on every state change. New pages: `PackageCustomiser` (toggle services, set addon quantities, event details form) and `MyRFQs` list + `RFQDetail` view. "Customise & request quote" enabled on catalogue cards; "My RFQs →" link surfaced. HR/Organiser nav gets a **My RFQs** menu item. Curl E2E: submit → list → cancel — all pass; player POST → 403.
- ✅ Iter 54 · Phase 1 (Jul 13, 2026): Corporate Services (RFQ module) — foundations.
